...
class TestCommitStep:
    ...
    def test_no_code_changes_skips_feat_commit(self, executor):
        # feat 게이트(첫 `diff --cached --quiet`)는 변경 없음(0), chore 게이트는 변경 있음(1).
        # _unstage_excluded의 `diff --cached --name-only` 호출과 구분하기 위해 --quiet 기준으로 카운트.
        quiet_count = {"n": 0}
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:3] == ("diff", "--cached", "--quiet"):
                quiet_count["n"] += 1
                return MagicMock(returncode=0 if quiet_count["n"] == 1 else 1)
            return MagicMock(returncode=0, stdout="", stderr="")
        executor._run_git = fake_git
...
# ---------------------------------------------------------------------------
# step{N}.result.json 기반 상태 보고 (하네스 견고화)
# ---------------------------------------------------------------------------

class TestStepResult:
    def test_preamble_forbids_index_edit_points_to_result(self, executor):
        result = executor._build_preamble("", "", step_num=2)
        assert "편집하지 마라" in result
        assert "step2.result.json" in result

    def test_read_missing_returns_none(self, executor):
        assert executor._read_step_result(2) is None

    def test_read_valid(self, executor):
        executor._result_path(2).write_text(
            json.dumps({"status": "completed", "summary": "done"}), encoding="utf-8"
        )
        res = executor._read_step_result(2)
        assert res == {"status": "completed", "summary": "done"}

    def test_read_corrupt_json_returns_none(self, executor):
        # 닫는 중괄호 누락 — 과거 crash를 유발하던 손상 패턴
        executor._result_path(2).write_text('{"status": "completed"', encoding="utf-8")
        assert executor._read_step_result(2) is None

    def test_read_without_status_returns_none(self, executor):
        executor._result_path(2).write_text(json.dumps({"summary": "x"}), encoding="utf-8")
        assert executor._read_step_result(2) is None

    def test_clear_removes_file(self, executor):
        p = executor._result_path(2)
        p.write_text("{}", encoding="utf-8")
        assert p.exists()
        executor._clear_step_result(2)
        assert not p.exists()

    def test_clear_missing_is_noop(self, executor):
        executor._clear_step_result(99)  # should not raise

    def test_apply_status_completed_writes_index(self, executor):
        executor._apply_step_status(2, "completed", "2026-01-01T00:00:00+0900", summary="ui done")
        idx = json.loads(executor._index_file.read_text())
        step2 = next(s for s in idx["steps"] if s["step"] == 2)
        assert step2["status"] == "completed"
        assert step2["summary"] == "ui done"
        assert step2["completed_at"] == "2026-01-01T00:00:00+0900"

    def test_apply_status_error_records_message(self, executor):
        executor._apply_step_status(2, "error", "2026-01-01T00:00:00+0900", error_message="boom")
        step2 = next(s for s in json.loads(executor._index_file.read_text())["steps"] if s["step"] == 2)
        assert step2["status"] == "error"
        assert step2["error_message"] == "boom"
        assert step2["failed_at"] == "2026-01-01T00:00:00+0900"

    def test_apply_completed_clears_stale_error(self, executor):
        executor._apply_step_status(2, "error", "t", error_message="old")
        executor._apply_step_status(2, "completed", "t2", summary="ok")
        step2 = next(s for s in json.loads(executor._index_file.read_text())["steps"] if s["step"] == 2)
        assert step2["status"] == "completed"
        assert "error_message" not in step2
